...
public abstract class TypeHelper {
    ...
    /**
     * @param n : 32bit로 변화하는 대상
     * @param offset : 시작위치
     * @param length : 가지고올 길이
     * @return
     * n을 32bit로 변환해서 offset 부터 length 만큼 가지고와서 Int로 변환한다.
     */
...
//	public static <T, E> Set<T> getKeysByValue(Multimap<T, E> map, E value) {
//	     Set<T> keys = new HashSet<T>();
//	     for (Entry<T, E> entry : map.entries()) {
//	         if (value.equals(entry.getValue())) {
...
//	}
    ...
    public static int getBitField(byte b, int field) {
    	return (b & (0x01 << field)) > 0 ? 1 : 0;
    }
    
    /**
     * 바이트 변수 b의 field번째 비트부터 count개수만큼의 비트 값을 리턴한다.
     * @param b : 바이트 변수
     * @param field : LSB부터 비트 번호 (0~7)
     * @param count : field부터 비트 개수 (LSB->MSB)
     * @return 비트 값. (0 < value < 255)
     * @author 김상복
     */
    ...
    /**
     * 비트 값 설정
     * 바이트 변수 b의 field번째 비트에 value값을 설정한다.
     * @param b : 바이트 변수
     * @param field : LSB부터 비트 번호 (0~7)
     * @param value : 설정하려는 값 (0 또는 1)
     * @return 비트 값이 적용된 바이트 변수
     * @author 한호연
     */
